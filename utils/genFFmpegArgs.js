...
module.exports = {
    genSingle: async (source, renditions, stream, output, hls_settings, video_id=-1, audio_id=-1, audio_filters="", escape_filters=false, watermark="", NVDEC_USE_SCALE=config.nvdec_use_scale, want_nvdec=true) => {
        ...
        */
        ...
        for (var i =0; i<renditions.length; i++) {
            ...
            if (rendition.hwaccel == "vaapi") {
                ...
                if (!is_start) {
                    ...
                    for (var j =0; j<render_devices.length; j++) {
                        try {
                            ...
                            if (va_check.includes('VAEntrypointEncSlice')) {
                                ...
                                try {                                    
                                    await check_output(config.ffmpeg, ['-loglevel', 'error', '-hwaccel', 'vaapi', '-vaapi_device', `${render_devices[j]}`, '-f', 'lavfi', '-i', 'testsrc', '-f', 'lavfi', '-i', 'testsrc', '-filter_complex', '[0:v]format=nv12,hwupload[a];[1:v]format=nv12,hwupload[b];[a][b]overlay_vaapi=x=W:y=H', '-vcodec', 'h264_vaapi', '-f', 'null', '-frames', '1', '-']) // i965 has no overlay support 
                                    vaSupportsOverlay = true
                                } catch (e) {}                                
                                break
                            }
                        } catch (e) {
                    ...
                    args.push((watermark && !vaSupportsOverlay) ? "nv12" : "vaapi")
        ...
        args.push(output.endsWith("/") ? output+"%v.m3u8" : output+"/%v.m3u8")
